...
namespace PETCENTER.MANTENIMIENTO.LOCALES.Proxy
{
    public class MaestrosProxyrest : ProxyBaseRest
    {
        ...
        {
            var url = ConfigurationManager.AppSettings["UrlConsultarEstadoFichaMantenimiento"];
            var response = DeserializarJSON<string, ConsultarEstadoFichaMantenimientoResponseDTO>("", url);
            if (response == null)
                throw new Exception(string.Format("Problemas con el servicio: {0}", url));
